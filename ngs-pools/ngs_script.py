...
from twistdb.db.models import *

# db = get_handle(engine_url="postgresql://twister:Of2dAd8cir5Y@db-qa/twistdb251")
db = get_handle(engine_url="postgresql://twister:Of2dAd8cir5Y@db-qa/twistdb_uat")
...
def reverse_complement(sequence):
    ...
    # print(sequence)
    raw_seq = sequence
    return raw_seq, str.translate(sequence, translation_table)[::-1]
...
summary_df = pd.concat([seq3.groupby('gc_bin').frequency.mean(),
                        target_profile, seq3['gc_bin'].value_counts()], axis=1)
